...
class AppStore extends Component {
  state = {searchInput: '', searchApp: tabsList[0].tabId}

  onUpdateTabId = tabId => {
    this.setState({searchApp: tabId})
  }
  ...
  filteredData = searchedApps => {
    const {searchApp} = this.state
    const filteredResult = searchedApps.filter(
      eachApp => eachApp.category === searchApp,
    )
  ...
  render() {
    ...
    const filteredResult = this.filteredData(searchResult)
          ...
          <ul className="list-order">
            {tabsList.map(eachTab => (
              <TabItem
                key={eachTab.tabId}
                eachTab={eachTab}
                onUpdateTabId={this.onUpdateTabId}
                isActive={eachTab.tabId === searchApp}
              />
            ))}
          </ul>
          <div>
            <ul className="list-items">
              {filteredResult.map(eachApp => (
